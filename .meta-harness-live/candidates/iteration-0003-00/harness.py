"""Balanced zero-shot: optimal token efficiency with semantic clarity."""
...
class Harness:
    ...
    def run(self, task, model, trace):
        ...
        trace.event("balanced_prompt", {"prompt": prompt, "labels": len(labels)})
        prediction = _pick(model(prompt), labels)
        if "label" in task:
            self.memory.append((query, str(task["label"])))
        trace.event("balanced_result", {"prediction": prediction})
        return prediction
